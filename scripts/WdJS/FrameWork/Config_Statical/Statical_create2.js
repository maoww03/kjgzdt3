                ...
                if(re.code == '200'){
                    if(re.data != null){
                        ...
                        for( var i=0;i<data.length;i++){
                            ...
                            if(aa == -1){
                                var items=new Sattical_createitem({_data:data[i],_num:i,_add:true,_navId:me._navId}).placeAt(me.items_module);
                                items._table2Fresh=me._table2Fresh.bind(me);
                                items.startup();
                                me._items.push(items);
                            }else{
                                var items=new Sattical_createitem({_data:data[i],_num:i,_add:false,_navId:me._navId}).placeAt(me.items_module);
                                items.startup();
                                me._items.push(items);
                            }
