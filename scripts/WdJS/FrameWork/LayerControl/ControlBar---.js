define([
  ...
  "dojo/_base/lang",
  "dojo/_base/array",
    "FrameWork/MapUtil/MapConfig", "FrameWork/MapUtil/MapConfig2"
], function (declare, template, TemplatedMixin, WidgetBase, on, domStyle, parser, domClass, domAttr, lang, array, MapConfig, MapConfig2) {
  return declare([WidgetBase, TemplatedMixin], {
    ...
    _createNodes: function () {
      ...
      //地图底图
        for (var i = 0; i < MapConfig2.layers.length; i++) {
            if (MapConfig2.layers[i].baseMap) {
          var basemapNodeData = {
              name: MapConfig2.layers[i].caption,
            isParent: true,
            nocheck: true,
              checked: MapConfig2.layers[i].visible,
            drag: false,
            nodeType: "basemap",
              basemapId: MapConfig2.layers[i].name,
          };
          var basemapNode = me._zTree.addNodes(basemapFolderNode, basemapNodeData)[0];
          for (var j = 0; j < appConfig.map2.getLayers().array_.length; j++) {
            var layer = appConfig.map2.getLayers().array_[j];
            if (layer.values_.baseMap) {
              if (layer.values_.groupCaption === MapConfig2.layers[i].caption) {
                var layerNodeData = {
    ...
    _addDiyDom: function (treeId, treeNode) {
      ...
      if (treeNode.nodeType === "basemap") {
        ...
        /*************20210407************ */
          ...
          for (var i = 0; i < MapConfig.layers.length; i++) {
            if (MapConfig.layers[i].baseMap) {
              if (MapConfig.layers[i].caption === parentNode.name) {
                dojo.byId("baseMapChangeSelect-logo").src = MapConfig.layers[i].logo;
      ...
      if (treeNode.nodeType === "mapLayer") {
        //  debugger
        var aObj2 = $("#" + treeNode.tId + "_slider");
