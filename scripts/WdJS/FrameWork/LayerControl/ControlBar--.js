...
], function (declare, template, TemplatedMixin, WidgetBase, on, domStyle, parser, domClass, domAttr, lang, array, MapConfig) {
    ...
    _createNodes: function () {
      ...
      for (var i = 0; i < MapConfig.layers.length; i++) {
        if (MapConfig.layers[i].baseMap) {
          var basemapNodeData = {
            name: MapConfig.layers[i].caption,
            isParent: true,
            nocheck: true,
            checked: MapConfig.layers[i].visible,
            drag: false,
            nodeType: "basemap",
            basemapId: MapConfig.layers[i].name,
          };
          ...
          for (var j = 0; j < appConfig.map.getLayers().array_.length; j++) {
            ...
            if (layer.values_.baseMap) {
              if (layer.values_.groupCaption === MapConfig.layers[i].caption) {
                var layerNodeData = {
