define([
  ...
  "dojo/dom-attr",
  "dojo/dom-construct",
  "FrameWork/serviceUtil/workMapService","FrameWork/Measure/GaussKruger","FrameWork/serviceUtil/excelWorkService","FrameWork/CustomizeShape2/CustomizeShape"
], function (declare, parser, DeferredList, _WidgetBase, _TemplatedMixin, template, on, domStyle, domClass, domAttr, domConstruct, workMapService, GaussKruger,excelWorkService,CustomizeShape) {
    return declare([_WidgetBase, _TemplatedMixin], {
        ...
        _bindEvents: function () {
            var me = this;
            // xzmb
            on(me.xzmb, "mouseover", function () {
               domStyle.set(me.xzmb,"background-color","#ccc");
            });
            on(me.xzmb, "mouseout", function () {
                domStyle.set(me.xzmb,"background-color","white");

            });
            // excelWorkService
            on(me.xzmb, "click", function () {
                var me=this;
                excelWorkService.downLoadExcelMB();
            ...
            // 4-upload_excel
            on(me.upload_excel, "click", function () {
            ...
            on(me.excel, "change", function () {
                
                var fd = new FormData();
                fd.append("file", me.excel.files[0]);
                var filePath = me.excel.value;
                var fileName = filePath.substring(filePath.lastIndexOf("\\") + 1); //文件名
                var extName = fileName.substring(fileName.lastIndexOf(".") + 1); //后缀名
                me._extName=extName;
                ...
                if (extName == "xls" || extName == "xlsx" ) {
                    domStyle.set(me.file_box,"display","block");
                    domStyle.set(me.upload_filename,"background-image","url(image/doConflict/excel.png)");
                    me.upload_filename.innerHTML = fileName;
                    me._getChangeTypeWkt(fd,extName);
                }else {
                    domStyle.set(me.file_box,"display","block");
            ...
            on(me.transformBtn, "click", function () {
                ...
                if (extName != "txt" && extName != "zip" && extName !='input' && extName !='xls' && extName !='xlsx') {
                        ...
                        if (out.code == 200) {
                            ...
                            if (typeSelect == "XY2BL") {
                                ...
                                out.data.forEach((d, index) => {
                                    debugger
                                    var newwkt=me._remove40.ShpIndataDeal(d.wkt);
                                    workMapService.XY2BL(newwkt, me._L0, 0).then(function (res) {
                                        performCount++;
                                        if (res.code == 200) {
                                            var attributes=[];
                   ...
                   var fd=me._excelFd;
                    if(fd){
                        excelWorkService.readExcel(fd).then(function (out) {
                            if (out.code == 200) {
                                me._filePath = out.data;
                                domStyle.set(me.loading, "display", "none");
                                domStyle.set(me.downloadBtn, 'display', 'block');
