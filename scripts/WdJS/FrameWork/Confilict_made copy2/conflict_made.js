define([
    ...
    "FrameWork/serviceUtil/graphService","FrameWork/serviceUtil/ConfigurationService","./resultTab","./layerConfig","./resultTab2"
], function (declare, parser, _WidgetBase, _TemplatedMixin, template, on, domStyle, domClass, domAttr, domConstruct, MapToolDraw, DeferredList, SuperMapWFSQuery, Survey,  uploadService, Loading,workMapService,graphService,ConfigurationService,resultTab,layerConfig,resultTab2) {
                        ...
                        for(var i=0;i<result.data.length;i++){          
                           ...
                           var feature = new ol.format.WKT().readFeature(newwkt);
                            ...
                            if(result.data[i].wkt.length>20000){
                                ...
                                console.log("count="+result.data[i].wkt);
                            }
            ...
            on(me.queryBtn, "click", function () {
                ...
                for (var j = 0; j < me._analyzeLayer.length; j++) {
                    ...
                    for (var k = 0; k < me._geom.length; k++) {
                        var _d = SuperMapWFSQuery.SpatialQuery( me._analyzeLayer[j].queryUrl, me._geom[k]);
                    ...
                    if (dataList.length > 0) {
                        ...
                        for (var i = 0; i < dataList.length; i++) {
                            ...
                            for (var j = 0; j < dataList[i].length; j++) {
                                ...
                                var _data;
                                if (field !== "") {
                                    _data = { type: dataList[i][j].properties.data[field],type2:dataList[i][j].properties.data[field2], area: 0,geometry:null };
                                } else {
